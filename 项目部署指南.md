...
```bash
# 克隆项目到本地
git clone https://github.com/ALPDM00/FaceAttendanceSystem.git
...
### 8.1 使用Windows服务部署
1. 使用NSSM（Non-Sucking Service Manager）将应用注册为Windows服务
2. 下载NSSM：https://nssm.cc/download
3. 安装服务：
   ```bash
   nssm install FaceAttendanceSystem
   # 设置路径为python.exe和main.py
   nssm set FaceAttendanceSystem Application python.exe
   nssm set FaceAttendanceSystem AppParameters src/main.py
   nssm set FaceAttendanceSystem AppDirectory E:\FaceAttendanceSystem
   ```
...
```bash
# 复制数据库文件
copy attendance_system.db attendance_system_backup.db
```
...
*Git仓库地址：https://github.com/ALPDM00/FaceAttendanceSystem.git*
